...
public class SmsBillByProductPolicy extends Policy {
	...
	public SmsBillByProductPolicy (ExportTask task){
		this.task = task;
		initPolicy();
	}
	...
	public void initPolicy() {
		this.setApiUrl("billing_sms_bill_by_product");
		...
		keys = new String[]{ "spCode", "spName", "platform", "city","productId","source","accountType","feeType",
				...
				"packageName","packageType","packageDesc","insideNum","outsidePrice"};
		...
		funcMap.put("packageType", new PeriodFunction());
		funcMap.put("source", new BillSourceFunction());

		initColumns(titles, keys, funcMap);
		this.setFileName(task.getExportTitle()+task.getId()+"."+task.getFileType());
		this.setSheetName(task.getExportTitle());
		this.setFilePath(getUnitPath("SmsBillByProductTask"));
	}
	...
	public Map<String, Object> getInfo(Map<String, Object> pageInfo) {
		...
		Map<String, Object> response = billApi.smsBillByProduct(JsonHelper.parseToJson(requestParams));
		return response;
	}
